...
#  rgb_linear = raw_image.postprocess(gamma=(1,1), no_auto_bright=True, output_bps=16)
  rgb_linear = raw_image.postprocess(no_auto_bright=True, output_bps=16)
  png_file = './linear_pngs/' + os.path.splitext(os.path.split(raw_file)[1])[0] + '.png'
#  imageio.imsave(tiff_file, rgb_linear)
  bgr_linear = cv2.cvtColor(rgb_linear,cv2.COLOR_RGB2BGR)
  cv2.imwrite(png_file, bgr_linear)
