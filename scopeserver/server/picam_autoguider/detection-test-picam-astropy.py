...
def find_guide_star(img_fn,img_ref_fn):
  ...
  return (guide_star_pos, guide_star_rect, median, bkg_sigma)


def find_star(img_fn,median,bkg_sigma):

  w = 128
  h = 128

  data = cv2.imread(img_fn,cv2.IMREAD_UNCHANGED)
  ...
  sources.sort(['peak'])
  sources.reverse()
  star_pos = np.array([sources['xcentroid'][0],sources['ycentroid'][0]])
...
(guide_star_pos, guide_star_rect, median, bkg_sigma) = find_guide_star(img_fns[0],img_ref_fn)
...
for img_fn in img_fns[1:]:
  ...
  star_pos = find_star(img_update_fn, median, bkg_sigma)
  dxi = star_pos[0]-w/2.
  dyi = star_pos[1]-h/2.
  dx += dxi
  dy += dyi
  print('%s : dX: %g  dY: %g' % (img_fn, dxi, dyi))
